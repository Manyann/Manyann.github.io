...
export class GestionComponent {
  ...
  desequipe(hero: string, armeCode: string) {
    this.armesService
      .desequipe(hero, armeCode)
      .then(() => this.handleSelect(hero));
  }
  ...
  equipeArmure(hero: string, armureCode: string) {
    this.armesService
      .equipeArmure(hero, armureCode)
      .then(() => this.handleSelect(hero));
  }

  desequipeArmure(hero: string, armureCode: string) {
    this.armesService
      .desequipeArmure(hero, armureCode)
      .then(() => this.handleSelect(hero));
  }
  ...
  supprimerArmure(hero: string, armureCode: string) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.armesService
          .removeArmureFromHero(hero, armureCode)
          .then(() => this.handleSelect(hero));
      },
    });
  }
  ...
  validerStats() {
    ...
    this.addStatsHero.forEach((element) => {
      this.herosService
        .updateStats(element['nom'], this.addKmParcourus, this.addOrs)
        .then((trophes) => this.handleTrophes(trophes));
    });
